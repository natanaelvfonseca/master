...
test("sync exige validação do escopo e da paginação por unidade", () => {
  assert.throws(
    () => assertFinancialSyncReady({ active: true, scope_verified: false, pagination_verified: true }),
    FinancialIntegrationStateError,
  );
  assert.throws(
    () => assertFinancialSyncReady({ active: true, scope_verified: true, pagination_verified: false }),
    FinancialIntegrationStateError,
  );
  assert.doesNotThrow(() =>
