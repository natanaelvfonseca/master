...
export const Route = createFileRoute("/financeiro/aluno/$studentId")({
  head: () => ({ meta: [{ title: "Perfil financeiro do aluno · Master" }] }),
...
function StudentFinancialProfile() {
  ...
  const load = React.useCallback(async () => {
    ...
    setLoading(true);
    try {
      const nextProfile = await readJson<Profile>(
          await fetch(
            `/api/financeiro/students/${encodeURIComponent(studentId)}?unit_id=${encodeURIComponent(unitId)}`,
            { credentials: "same-origin" },
          ),
        );
      if (activeUnitId.current === unitId && requestVersion.current === version) {
  ...
  if (profile && loadedUnitId !== unitId) return <div className="grid min-h-64 place-items-center"><Loader2 className="animate-spin text-primary" /></div>;
  if (loading && !profile)
    return (
