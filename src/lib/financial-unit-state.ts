...
export function assertFinancialSyncReady(integration: { active: boolean; scope_verified: boolean; pagination_verified: boolean } | null) {
  if (!integration)
  ...
  if (!integration.scope_verified || !integration.pagination_verified)
    throw new FinancialIntegrationStateError("Confirme o escopo do token e a cobertura/paginação do CAEZ antes de sincronizar.");
}
