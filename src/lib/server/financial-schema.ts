...
create table if not exists app_financial_integrations (
  ...
  active boolean not null default true, scope_verified boolean not null default false,
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'app_financial_enrollments_student_unit_fk') then
    alter table app_financial_enrollments add constraint app_financial_enrollments_student_unit_fk
  foreign key (unit_id, student_id) references app_financial_students (unit_id, id);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'app_financial_installments_student_unit_fk') then
    alter table app_financial_installments add constraint app_financial_installments_student_unit_fk
  foreign key (unit_id, student_id) references app_financial_students (unit_id, id);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'app_financial_installments_enrollment_unit_fk') then
    alter table app_financial_installments add constraint app_financial_installments_enrollment_unit_fk
  foreign key (unit_id, enrollment_id) references app_financial_enrollments (unit_id, id);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'app_financial_actions_student_unit_fk') then
    alter table app_financial_collection_actions add constraint app_financial_actions_student_unit_fk
  foreign key (unit_id, student_id) references app_financial_students (unit_id, id);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'app_financial_actions_enrollment_unit_fk') then
    alter table app_financial_collection_actions add constraint app_financial_actions_enrollment_unit_fk
  foreign key (unit_id, enrollment_id) references app_financial_enrollments (unit_id, id);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'app_financial_actions_installment_unit_fk') then
    alter table app_financial_collection_actions add constraint app_financial_actions_installment_unit_fk
  foreign key (unit_id, installment_id) references app_financial_installments (unit_id, id);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'app_financial_promises_student_unit_fk') then
    alter table app_financial_promises add constraint app_financial_promises_student_unit_fk
  foreign key (unit_id, student_id) references app_financial_students (unit_id, id);
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'app_financial_promises_installment_unit_fk') then
    alter table app_financial_promises add constraint app_financial_promises_installment_unit_fk
  foreign key (unit_id, installment_id) references app_financial_installments (unit_id, id);
  end if;
end $$;
`;
