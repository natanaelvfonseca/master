...
async function request<T>(baseUrl: string, token: string, path: string, query?: URLSearchParams) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (!Array.isArray(payload.dados)) throw new Error("Resposta CAEZ sem lista de dados.");
      const total = Number(payload.total_registros ?? payload.dados.length);
      if (!Number.isFinite(total) || total > payload.dados.length)
        throw new Error("CAEZ retornou uma página incompleta; sincronização interrompida.");
      return {
        total,
        data: payload.dados,
      };
  ...
  if (lastError instanceof Error && /CAEZ retornou uma página incompleta|Resposta CAEZ sem lista de dados/.test(lastError.message))
    throw lastError;
  throw new Error(
